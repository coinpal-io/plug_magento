...
class Pay extends \Magento\Payment\Model\Method\AbstractMethod
{
	...
	public function getGlocashUrl($quote,$orderId){
        ...
        $param = array(
            ...
            'successUrl'=>$this->helper->getUrl('glocash/standard/responsepay')."?invoice=".$quote->getReservedOrderId().'&t=s&CUS_EMAIL='.$quote->getCustomerEmail(),
            'redirectURL'=>$this->helper->getUrl('glocash/standard/responsepay')."?invoice=".$quote->getReservedOrderId().'&t=s&CUS_EMAIL='.$quote->getCustomerEmail(),
            'cancelURL'=>$this->helper->getUrl('glocash/standard/fail')."?invoice=".$quote->getReservedOrderId().'&t=f',
            'notifyURL'=>$this->helper->getUrl('glocash/standard/responsenotify'),
        );
...
//        $gatewayUrl = "https://pay-dev.coinpal.io/gateway/pay/checkout";
		...
		if ($httpCode!=200 || empty($datas['nextStepContent'])) {
			...
			$action=$this->helper->getUrl('glocash/standard/fail')."?invoice=".$quote->getReservedOrderId().'&t=f&error='.$datas['respMessage'];
		} else {
    ...
    public function postProcessing(\Magento\Sales\Model\Order $order, \Magento\Framework\DataObject $payment, $response) {
		...
		Logs::logw("#".$response['orderNo']." Successful modification of order status,ID:".$order->getId()." Number:".$response['orderNo']." Status:".$this->getOrderStatus(),"glocash.log","Order_modification");
